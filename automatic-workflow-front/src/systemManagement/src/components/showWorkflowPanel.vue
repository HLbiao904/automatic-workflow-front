<template>
  <el-dialog
    ...
    width="90%"
    top="2vh"
    :title="props.previewMode == true ? '工作流版本预览' : '工作流执行记录预览'"
...
import { layoutNodes } from "../utils/tools.js";
...
watch(
  ...
  (val) => {
    ...
    // 彻底断引用
    nodes.value = JSON.parse(JSON.stringify(rawNodes));
    edges.value = JSON.parse(JSON.stringify(rawEdges));
